...
import { configure, services, withApiKey } from "orangeslice";
import { createServer } from "http";
import { z } from "zod";

// Per-connection auth: callers send their own key in the X-Orangeslice-Api-Key
// header and it is scoped to that request only (see the http handler below).
// ORANGESLICE_API_KEY, when set, is the fallback for requests without one.
const API_KEY_HEADER = "x-orangeslice-api-key";
const DEFAULT_API_KEY = process.env.ORANGESLICE_API_KEY || "";

if (DEFAULT_API_KEY) {
  configure({ apiKey: DEFAULT_API_KEY });
} else {
  console.error(
    `No ORANGESLICE_API_KEY set — every request must carry its own key in the ${API_KEY_HEADER} header.`
  );
}

// A fresh McpServer + transport is built per HTTP request: the MCP SDK allows
// only one transport per server instance, so a shared instance breaks as soon as
// two requests overlap — which is the normal case once each caller brings its
// own API key.
...
const httpServer = createServer(async (req, res) => {
  ...
  const requestApiKey = String(req.headers[API_KEY_HEADER] || "").trim();
  if (!requestApiKey && !DEFAULT_API_KEY) {
    res.writeHead(401, { "Content-Type": "application/json" });
    res.end(
      JSON.stringify({
        error: `No OrangeSlice API key. Send your own key in the ${API_KEY_HEADER} header, or set ORANGESLICE_API_KEY on the server.`,
      })
    );
    return;
  }
  ...
  // withApiKey scopes the caller's key to this request via AsyncLocalStorage,
  // so concurrent requests from different users never see each other's key.
  if (requestApiKey) {
    await withApiKey(requestApiKey, () => transport.handleRequest(req, res, parsedBody));
  } else {
